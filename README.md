...
**有网机器**
```bash
dpack install dp                # 自动检测 GPU/CUDA，下载 + 安装
```
...
│
└─ 📁 references/          开发内部文档（用户无需阅读）
   ├─    notes.md           排查手册 & GPU / commit 流程
   └─    verification-log.md 验证记录（在 Bohrium 实测的结果与踩坑）
```
...
> **GPU 用户只需一个包**：GPU 版（`cuda129`）覆盖**整个 CUDA 12.x ~ 13.0 驱动**——
> 不管你的驱动是 12.6、12.8 还是 13.0，装这一个就行（靠
> [NVIDIA 向后兼容](https://docs.nvidia.com/deploy/cuda-compatibility/)），不用对版本。
...
bash scripts/build.sh --cuda 12.9     # GPU（覆盖 12.x~13.0 驱动）
...
**Online machine**
```bash
dpack install dp                # auto-detect GPU/CUDA, download + install
```
